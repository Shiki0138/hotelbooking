...
```bash
# マルチエージェント確認
tmux attach-session -t hotel_multiagent

# プレジデント確認（別ターミナルで）
tmux attach-session -t hotel_president
```
...
```bash
# まずPRESIDENTで認証を実施
tmux send-keys -t hotel_president 'claude --dangerously-skip-permissions' C-m
```
...
for i in {0..3}; do tmux send-keys -t hotel_multiagent:0.$i 'claude --dangerously-skip-permissions' C-m; done
```
...
# ペイン一覧
tmux list-panes -t SESSION_PRE2
tmux list-panes -t SESSION_MAIN2
```
...
```bash
# セッション削除
tmux kill-session -t SESSION_PRE2
tmux kill-session -t SESSION_MAIN2
